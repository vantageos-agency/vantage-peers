...
# v1.3.3 (2026-07-10, Sigma) — heredoc bodies only. NO anchoring.
#
# A previous v1.3.3 candidate (sha256 2caf9cef...) also anchored detection to a
# segment-leading `[VAR=val ...] (npm|pnpm|yarn|bun) publish`. It was WITHDRAWN
# before review, on measurement: anchoring fail-opened on six real publishes
# (`sudo npm publish`, `env npm publish`, `( npm publish )`,
# `if true; then npm publish; fi`, `npx npm publish`, `bash -c '...'`), of which
# the plain substring patterns above caught five. Scored against the same
# adversarial set: original 8/10, +strip_heredocs 9/10, +anchoring 4/10.
#
# The anchoring bought nothing: the two false positives it claimed to fix
# (`gh pr create --title "... npm publish"`, `git commit -m "... npm publish"`)
# are already neutralised by strip_quoted_strings (v1.0.1). The ONLY genuine
# false positive was the heredoc commit body — which strip_heredocs alone fixes.
#
# A security gate is judged on its FALSE NEGATIVES. Tests that only cover the
# error class you set out to fix measure nothing.
#
# Known residual hole, pre-dating this change, tracked as
# k174017wa5vnckx570e36d8nqd8a9e5g: `bash -c 'npm publish'` escapes ALL variants
# because strip_quoted_strings erases the interpreter's quoted argument — i.e.
# the real command — before detection runs.
...
# ─────────────────────────────────────────────────────────────────────────────
# strip_heredocs — v1.3.3 (2026-07-04, Sigma)
# A heredoc body (`git commit -F - <<'EOF' ... npm publish ... EOF`) is NOT a
# publish invocation, but its text is not inside '' or "" so strip_quoted_strings
# leaves it. Remove `<<[-]['"]?TAG['"]? ... TAG` blocks before matching.
# ─────────────────────────────────────────────────────────────────────────────
...
def is_fleet_publish(command: str) -> bool:
    """Returns True if command mentions a publish verb targeting a fleet package.
    Heredoc bodies and quoted strings are stripped first, then the plain
    substring patterns apply. v1.3.3 (Sigma): adds strip_heredocs, which kills
    the heredoc-commit-body false positive. Detection stays deliberately BROAD —
    a security gate must fail CLOSED, so `sudo npm publish`, `npx npm publish`,
    `( npm publish )` and friends all match."""
    sanitized = strip_quoted_strings(strip_heredocs(command))
    has_publish = any(re.search(p, sanitized) for p in PUBLISH_CMD_PATTERNS)
    if not has_publish:
        return False
