...
server.tool(
		...
		"fields='lite' returns compact payloads ({_id,title,status,priority,assignedTo,missionId}) — fewer tokens.",
	{
		...
		status: z
			...
			.optional()
			.describe(
				"Filter by status. Single value, array, or alias: " +
...
server.tool(
		...
		"fields='lite' returns compact payloads ({_id,title,status,priority,assignedTo,missionId}) — fewer tokens.",
	{
		...
		status: z
			...
			.optional()
			.describe(
				"Filter by task status. Single value, array, or alias: " +
				"'open' (non-terminal), 'active' (in_progress only), 'all' (no filter).",
			),
		fields: z
			.enum(["lite", "full"])
...
server.tool(
	"delete_diary",
	"Permanently delete a diary entry by ID. Only the owner (or system) can delete.",
	{
	...
	async ({ diaryId, callerOrchestrator }) => {
		try {
			const result = await convex.mutation("diary:deleteDiary" as any, {
				diaryId: diaryId as any,
				callerOrchestrator,
			});
...
server.tool(
	"delete_briefing_note",
	"Permanently delete a briefing note by ID. Only the creator (or system) can delete.",
	{
	...
	async ({ noteId, callerOrchestrator }) => {
		try {
			const result = await convex.mutation("briefingNotes:deleteBriefingNote" as any, {
				noteId: noteId as any,
				callerOrchestrator,
			});
