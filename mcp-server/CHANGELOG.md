...
- **`list_components`, `register_component`, `get_component`, `update_component`, `delete_component`, `search_components` — removed (task `k173r2p1yh94m5f7yvgr1b30gx8dn3ez`, operator ruling).** The `components` registry (an agent/skill/hook/plugin catalogue) served zero rows in production and one orchestrator smoke-test residue row in DEV; the six tools are removed from `src/tools.ts`, their entry is removed from the published package's hand-maintained `api.ts` API mirror, and the smoke walk in `scripts/test-mcp.ts` no longer calls them. `src/auth.ts`'s `bearerAuthMiddleware` also drops the two legacy bearer-auth fall-through branches after the Clerk-JWT layer (2.5): the DCR opaque-token layer (`oauthTokens`/`oauthClients`, backed by the now-deleted `convex/oauthDcr.ts`) and the legacy internal-bearer layer (`mcpTenants`, backed by the now-deleted `convex/mcpTenants.ts`). A bearer token shaped like either removed credential is now refused outright (401) — never a fall-through grant. `scripts/seed-mcp-tenant.ts`, a CLI that existed only to provision the removed `mcpTenants` table, is deleted. Served tool count, derived by running `registerTools()` against a fake server that records every registration name (`npm run build` then a one-off Node harness): **113 → 107**. Every test that existed only for the removed surface was deleted (`src/__tests__/list_components.tool.test.ts`, `test/c0-2-component-gate.test.ts`, `src/__tests__/dcr-scope-enforcement.test.ts`) or rewritten to prove the DENY pole (`src/__tests__/oauth-scoped.test.ts`'s DCR-path and legacy-mcpTenants describe blocks now assert 401 + the removed lookup is never attempted, rather than the old PASS-pole behaviour). `mcp-server vitest run`: 107/107 files, 1818/1830 tests, 12 skipped, 0 failed; `tsc --noEmit` 0.

### Fixed

- **`check_messages` now renders `actionableStuckCount` alongside `stuckInProgress` / `peersStuckOnYou`.** Convex's `computeStuckList` already derived this count (entries whose OPEN segment is strictly past the configured threshold), but `asCappedStuckList` only read `entries`/`total`/`truncated` off the envelope object and silently dropped the fourth field before it reached the rendered text — the count was computed on every cycle and never surfaced. Fixed by threading `actionableStuckCount` through `asCappedStuckList`, defaulting to `0` (missing key, raw array shape, or an older Convex whose capped object predates the field) rather than throwing or going `undefined`, matching the function's existing reader-first tolerance. Consumer test drives the real `check_messages` handler over real Convex functions with a row pinned exactly at the configured threshold (fake timers on `Date` only) to make the `>` boundary of the underlying condition load-bearing, bracketed by a row strictly past the threshold so the assertion cannot pass on a count that is always zero.
