...
try {
    console.log(`[vantage-peers-mcp] Running auto-migrations for v${pkg.version}…`);
    const migrationResult = await internalClient().mutation(
    // biome-ignore lint/suspicious/noExplicitAny: Convex string API
    "migrationRunner:applyPendingMigrations", { callerVersion: pkg.version });
    const result = migrationResult;
    if (result.applied > 0) {
        console.log(`[vantage-peers-mcp] Migrations applied: ${result.appliedVersions.join(", ")}`);
    }
    else {
        console.log(`[vantage-peers-mcp] Migrations: already up-to-date (${result.alreadyApplied.length} applied previously)`);
    }
}
catch (err) {
    const message = err instanceof Error ? err.message : String(err);
