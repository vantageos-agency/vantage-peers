 ...
 */
import { VpToolResultSchema } from "./schemas.js";
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export function wrapToolResult(payload) {
    const result = VpToolResultSchema.safeParse(payload);
    if (!result.success) {
        throw new TypeError(`[stream-marker] wrapToolResult: invalid payload — ${result.error.message}`);
    }
