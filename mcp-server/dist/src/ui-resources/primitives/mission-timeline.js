...
export async function renderMissionTimeline(query, fetchConvex) {
    ...
    let missions = [];
    try {
        const result = (await fetchConvex("missions:list", args));
  ...
  <p class="vp-mission-empty">${esc(labels.empty)}</p>
    ...
    const rows = missions
        ...
        const progressPct = m.progress !== undefined
            ...
            : null;
        const progressCell = progressPct !== null
