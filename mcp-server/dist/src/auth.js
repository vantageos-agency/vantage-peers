 ...
 */
export function checkNamespacePrefix(prefixes, namespace) {
...
export function checkNamespaceRead(ctx, namespace) {
    if (!ctx)
        return null;
    if (isMasterScope(ctx))
        return null;
...
export function checkNamespaceWrite(ctx, namespace) {
    if (!ctx)
        return null;
    if (isMasterScope(ctx))
        return null;
    if (checkNamespacePrefix(ctx.namespaceWritePrefixes, namespace))
        return null;
...
export function bearerAuthMiddleware() {
    return async (c, next) => {
        ...
        try {
            ...
            "oauth:getAccessTokenByHash", { tokenHash }));
        }
        catch (err) {
        ...
        if (oauth) {
            const internalUrl = process.env.CONVEX_URL_INTERNAL;
            if (!internalUrl) {
                console.error("[auth] CONVEX_URL_INTERNAL not set — cannot route OAuth token");
                return c.json({ error: "Server misconfigured: internal deployment URL missing" }, 500);
            }
            c.set("tenant", {
        ...
        try {
            ...
            // biome-ignore lint/suspicious/noExplicitAny: Convex string API
            "mcpTenants:getTenantByTokenHash", { tokenHash }));
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
