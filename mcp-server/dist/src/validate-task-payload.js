...
// ─────────────────────────────────────────────────────────────────────────────
function checkTimeEstimate(text, acc) {
    ...
    acc.errors.push({
        ...
        copy_paste_fix: "Remove the estimate. Replace with 'TBD' or omit the line entirely.\n" +
...
// ─────────────────────────────────────────────────────────────────────────────
function checkVerificationTests(description, acc) {
    ...
    if (missingVerification) {
        ...
        acc.warnings.push("AUTO-INJECT: VERIFICATION: section missing — added placeholder. " +
            "Replace 'TBD' with actual acceptance criteria before closing the task.");
    }
    if (missingTests) {
        acc.auto_inject.push("description.TESTS");
        acc.warnings.push("AUTO-INJECT: TESTS: section missing — added placeholder. " +
    ...
    if (missingVerification || missingTests) {
        ...
        const current = acc.patched.description ?? description;
        let patched = current;
        if (missingVerification) {
            patched += "\n\nVERIFICATION: TBD";
        }
...
// ─────────────────────────────────────────────────────────────────────────────
const DELEGATION_KEYS = ["subagent_type", "run_in_background", "model"];
function checkDelegationTriplet(description, acc) {
    // Skip if [META] tag present (orchestrator meta-tasks exempt from delegation rules)
    if (/\[META\]/i.test(description))
        return;
    const lower = description.toLowerCase();
    ...
    if (present.length === 0)
        return; // no delegation intent — not required
    const missing = DELEGATION_KEYS.filter((k) => !present.includes(k));
    if (missing.length === 0)
        return; // all three present — valid
    acc.errors.push({
        field: "delegation.triplet",
        message: `Partial delegation triplet detected — present: [${present.join(", ")}], ` +
            `missing: [${missing.join(", ")}]. All three fields are required together.`,
        copy_paste_fix: "Add all three delegation fields to the description:\n" +
...
function checkEvidence(note, acc) {
    if (note.includes(OPT_OUT_EVIDENCE))
        return;
    if (!note.trim()) {
        acc.errors.push({
            field: "completionNote.length",
            message: "completionNote is missing or empty.",
            copy_paste_fix: "Add a completionNote with ≥40 chars and at least one evidence token:\n" +
    ...
    if (note.trim().length < 40) {
        acc.errors.push({
            ...
            copy_paste_fix: "Expand the note to ≥40 chars. Include at least one evidence token:\n" +
    ...
    if (!hasEvidence(note) && CLAIM_WORDS_RE.test(note)) {
        acc.errors.push({
            ...
            message: "completionNote uses claim words ('done', 'merged', 'deployed'…) without verifiable evidence.",
            copy_paste_fix: "Replace or extend with a verifiable token. Examples:\n" +
        ...
        acc.errors.push({
            ...
            copy_paste_fix: "Add at least one verifiable token to the note. Examples:\n" +
                "  PR #42 merged. 15/15 tests passing. commit d8ceef5.\n" +
                "  https://github.com/.../pull/42\n" +
                "Opt-out (no artifact): add `// allow-no-evidence: <reason>` in the note.",
        });
...
function checkFriction(note, acc) {
    if (FRICTION_OPT_OUT_RE.test(note))
        return;
    if (FRICTION_LINE_RE.test(note))
        return;
    // Auto-inject: append friction_observed: none to modified completionNote
    acc.auto_inject.push("friction_observed");
    acc.warnings.push("AUTO-INJECT: friction_observed missing — added 'friction_observed: none' placeholder. " +
    ...
    const current = acc.patched.completionNote ?? note;
...
function checkMessageTaskRef(channel, content, acc) {
    if (CHANNEL_EXEMPT.has(channel))
        return;
    if (!content.trim())
        return;
    ...
    if (content.length < 120 && !hasVerb && !hasActionList)
        return;
    // Check exemptions
    ...
    if (!hasVerb && !hasActionList)
        return;
    acc.errors.push({
        field: "content.task_ref",
        message: "Message contains action instructions without a task reference or info marker.",
        copy_paste_fix: "Option A — prefix content with an info marker (if informational):\n" +
...
// Per-tool dispatch
// ─────────────────────────────────────────────────────────────────────────────
function dispatchCreateOrUpdateTask(toolName, payload, acc) {
    ...
    if (toolName === "update_task" && description === null) {
        ...
        if (status === "review" || status === "done") {
            const note = typeof payload.completionNote === "string"
    ...
    if (toolName === "create_task" && !description) {
        acc.errors.push({
            ...
            copy_paste_fix: "Add a description field:\n" +
...
function dispatchCompleteTask(payload, acc) {
    const note = typeof payload.completionNote === "string" ? payload.completionNote : "";
    // Time estimate check
...
function dispatchSendMessage(payload, acc) {
 ...
 */
export function validateTaskPayload(toolName, payload) {
