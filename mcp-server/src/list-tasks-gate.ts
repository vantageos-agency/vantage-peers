 ...
 * oauthCtx.userId (profile name "helios-acme-hr") instead of fromAllowList.
 *
 ...
 */
export function listTasksGate(
	oauthCtx: OAuthContext | undefined,
	assignedTo: string | undefined,
	createdBy: string | undefined,
): string | null {
	if (!oauthCtx || isMasterScope(oauthCtx)) return null;
	...
	if (assignedTo === undefined && createdBy === undefined) return null;
	...
	const allowed =
		fromAllowList.length > 0
			? fromAllowList.some(
					(a) => normalizeOrchestratorId(a) === normPresented,
				)
	...
	if (!allowed) {
		...
		return (
			`Forbidden: list_tasks requires assignedTo or createdBy ∈ fromAllowList ` +
