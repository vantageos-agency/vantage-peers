 ...
 * asserts that all 84 VP MCP tools register an `annotations` object with
...
// ─────────────────────────────────────────────────────────────────────────────
// Expected categorization — 34 readOnly + 41 write + 9 destructive = 84
// ─────────────────────────────────────────────────────────────────────────────
...
describe("ChatGPT Apps SDK tool annotations (Day 88)", () => {
	...
	it("all 34 read-only tools have readOnlyHint=true and destructiveHint=false", () => {
		...
		expect(READ_ONLY_TOOLS.size).toBe(34);
	});

	it("all 9 destructive tools have destructiveHint=true and readOnlyHint=false", () => {
		const { server, tools } = buildCapturingServer();
		registerTools(server, buildMockConvex());
		...
		expect(mismatches, mismatches.join("\n")).toEqual([]);
		expect(DESTRUCTIVE_TOOLS.size).toBe(9);
	});

	it("all 41 write tools have readOnlyHint=false and destructiveHint=false", () => {
		const { server, tools } = buildCapturingServer();
		registerTools(server, buildMockConvex());
		...
		expect(mismatches, mismatches.join("\n")).toEqual([]);
		expect(writeTools.length).toBe(41);
	});
