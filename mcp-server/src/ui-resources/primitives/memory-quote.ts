...
export async function renderMemoryQuote(
	...
	const heading =
		lang === "fr" ? "Mémoires VantagePeers" : "VantagePeers Memories";
	const emptyLabel =
		lang === "fr" ? "Aucune mémoire trouvée." : "No memories found.";
	...
	try {
		const args: Record<string, unknown> = { namespace, limit };
		if (type) args.type = type;
		const result = (await fetchConvex(
			"memories:listMemories",
			args,
		)) as ListMemoriesResult;
