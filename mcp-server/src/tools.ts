...
export const BULK_COMPLETE_TASKS_TOOL_DESCRIPTION =
	...
	"HARD CAP: maximum 500 tasks per call — if matched count exceeds 500 the mutation throws BULK_HARD_CAP_EXCEEDED. Narrow your filter and retry. " +
	"REDUCTIVE FILTER REQUIRED: at least one of autoGeneratedOnly=true OR assignedTo=<name> must be set — an empty filter throws BULK_FILTER_TOO_BROAD. " +
	...
	"Returns {count, sampleIds, bulkRunId, executedAt?, cappedAt?} — bulkRunId is the Day-76 evidence token; cappedAt is present when dry-run preview was truncated to 500.";
