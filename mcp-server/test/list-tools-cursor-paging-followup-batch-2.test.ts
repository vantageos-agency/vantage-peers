/**
 * S3.3 B8 follow-up batch 2 — additive cursor paging rollout to 6 more list_* tools.
 *
 * Parent PR #635 + follow-up batch 1 PR #637 wired 9 list_* tools to the shared
 * `paging.ts` utility (DEFAULT_LIMIT=50, MAX_LIMIT=200, ENVELOPE_TARGET_BYTES=50000,
 * clampLimit / encodeCursor / decodeCursor / enforceEnvelopeCap / buildPageResult).
 * This follow-up batch applies the same backward-compatible pattern to:
 *
 *   1. list_messages           (convex/messages.ts: listMessages)
 *   2. list_tasks_by_mission   (convex/tasks.ts: listByMission)
 *   3. list_repo_mappings      (convex/githubRepoMapping.ts: list)
 *   4. list_issues             (convex/issues.ts: listByProject)
 *   5. list_fix_patterns       (convex/fixPatterns.ts: listAll)
 *   6. list_errors             (convex/errorMonitor.ts: listErrors)
 *
...
// list-tools-cursor-paging-followup-batch-1.test.ts).
// ─────────────────────────────────────────────────────────────────────────────
...
// generate a uniform 6-test block per tool (default forward, explicit limit,
...
const TOOLS: ToolSpec[] = [
	{ mcpName: "list_messages", convexFn: "messages:listMessages", baseArgs: {} },
	{
		mcpName: "list_tasks_by_mission",
		convexFn: "tasks:listByMission",
		baseArgs: { missionId: "k57c7s478gw1a3e5gmhdeptg5n87z78n" },
	},
	{
		mcpName: "list_repo_mappings",
		convexFn: "githubRepoMapping:list",
		baseArgs: {},
	},
	{
		mcpName: "list_issues",
		convexFn: "issues:listByProject",
		baseArgs: {},
	},
	{
		mcpName: "list_fix_patterns",
		convexFn: "fixPatterns:listAll",
		baseArgs: {},
	},
	{
		mcpName: "list_errors",
		convexFn: "errorMonitor:listErrors",
		baseArgs: {},
	},
];

for (const spec of TOOLS) {
	describe(`${spec.mcpName} — cursor paging wiring (S3.3 B8 follow-up batch 2)`, () => {
