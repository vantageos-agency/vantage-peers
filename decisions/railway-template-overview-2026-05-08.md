...
2. **Add environment variables** — two layers, each with its own set:
   - **In Railway** (Variables tab): `NODE_ENV=production`, `CONVEX_URL_INTERNAL` (your `https://…convex.cloud` URL), `BEARER_SECRET_MASTER`, `PUBLIC_BASE_URL=https://${{ RAILWAY_PUBLIC_DOMAIN }}` (Railway interpolates `${{ RAILWAY_PUBLIC_DOMAIN }}` to the bare host — no scheme — so the literal `https://` prefix is required; otherwise the OAuth metadata fallback emits `host/.well-known/…` which is not an absolute URL per RFC 9728. OAuth metadata discovery falls back to this value when the `Host` header is absent — e.g. `curl` smoke without `-H "Host: …"`). Optionally add `CLERK_SECRET_KEY` and `CLERK_PUBLISHABLE_KEY`.
