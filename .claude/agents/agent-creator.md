...
4. **Inject Error Handling Sections**: Before writing the agent file, inline both canonical error-handling sections from the fleet templates into the system prompt body. The sections MUST appear between `## PERSONA` and `## SCOPE BOUNDARY` in this exact order:

   1. Read `resources/templates/agent-input-validation-template.md` and copy the verbatim `## INPUT VALIDATION` section into the agent body.
   2. Read `resources/templates/agent-failure-recovery-template.md` and copy the verbatim `## FAILURE RECOVERY` section into the agent body, immediately after `## INPUT VALIDATION`.
   3. Customize both sections per archetype:
      - **Specialist** (code-reviewer, seo-technical, data-analyst, ...): adapt validation for URL/file/API inputs; adapt failure recovery with fallback data sources and per-item batch recovery.
      - **Orchestrator** (delivery-manager, agent-creator, ...): adapt validation for VantagePeers task/mission IDs and peer existence; adapt failure recovery for subagent dispatch errors and MCP unavailability.
      - **Reviewer** (deliverable-reviewer, skill-reviewer, ...): adapt validation for artifact existence and schema compliance; adapt failure recovery for partial rubric completion.
      - **Researcher** (tech-researcher, market-researcher, ...): adapt validation for URL reachability and source count; adapt failure recovery for source fallbacks and citation gaps.
      - When in doubt, fall back to the scenario list in `resources/templates/agent-failure-recovery-template.md` — the generic template is always valid.
   4. Quality Self-Check before writing: Does the new agent contain `## INPUT VALIDATION` AND `## FAILURE RECOVERY` sections positioned between `## PERSONA` and `## SCOPE BOUNDARY`? If not, add them before proceeding. If the agent is a legitimate exception (pure documentation stub, migration shim), add `// allow-agent-no-error-handling: <reason>` to the body and note it in the summary.

5. **Generate Agent File**: Use Write tool to create `agents/[identifier].md`:
   ```markdown
   ---
   name: [identifier]
   ...
   [Complete system prompt with ## INPUT VALIDATION and ## FAILURE RECOVERY
    between ## PERSONA and ## SCOPE BOUNDARY]
   ```
