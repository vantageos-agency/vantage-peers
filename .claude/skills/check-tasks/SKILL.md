...
3. Extract instanceId from CLAUDE.md (e.g., `sigma-vps-2`) or fall back to hostname.
...
If the user passes an `instanceId` (e.g., "tasks for pi-chromebook"), additionally call `list_tasks` with `assignedToInstance=<instanceId>`, same `fields=lite` + `limit=20`, and merge dedup by `_id`.
