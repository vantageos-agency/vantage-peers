...
def run_tests(test_dir: str, subset: str | None = None) -> tuple[bool, int, int, int, str]:
    """
    Run `npm test` (or subset) in test_dir.
    Return (all_green, passed, failed, skipped, raw_output_tail).
    """
    env = os.environ.copy()
    ...
    try:
        ...
        if not m:
            ...
            return all_green, 0, 0, 0, output
        failed = int(m.group(1) or 0)
        passed = int(m.group(2) or 0)
        skipped = int(m.group(3) or 0)
        all_green = (failed == 0 and r.returncode == 0)
        return all_green, passed, failed, skipped, output
    except subprocess.TimeoutExpired:
        return False, 0, 0, 0, "TIMEOUT after 180s"
    except (subprocess.CalledProcessError, FileNotFoundError) as e:
        return False, 0, 0, 0, f"ERROR running tests: {e}"
...
def main() -> int:
    ...
    all_green, passed, failed, skipped, output_tail = run_tests(test_dir, subset)

    if all_green:
        log(f"PASS {passed} passed, {skipped} skipped — allow")
        return 0
