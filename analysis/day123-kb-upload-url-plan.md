...
`store_document_chunked` (kb.ts:184) exige un `storageId` d'un blob **déjà uploadé**, mais **rien n'expose `ctx.storage.generateUploadUrl()`** → aucun client (Nadia incluse) ne peut obtenir de `storageId` → **personne ne peut uploader** → cause du prod 0-doc (`npx convex data _storage` et `kbUploads` = vides). En amont du fix d'indexation #1056 (déjà en prod, 99c5cbf), qui reste correct mais inerte sans upload.
