...
V3 PRINCIPLE — NEW MISSION ⇒ `mission-bootstrap`. When Laurent's stated goal is "start a mission for X" / "scaffold mission Y" / "new mission Z", do not call `create_mission` directly. Chain to the `mission-bootstrap` skill, which wires VERIFICATION/TESTS-compliant child tasks and pre-satisfies the task-quality hook.
...
- If header says "You are Pi" AND workspace path is `/home/laurentperello/coding/ElPi Corp` (Chromebook) → **HUMAN MODE**.
- Else (any VPS orchestrator: sigma, alpha, lambda, victor, tau, phi, omega, eta, zeta, proxima, verify, scan, etc.) → **AUTONOMOUS MODE**.
- In doubt, default to autonomous mode.
...
Ask Laurent: "What do you want to accomplish today beyond routines?" Wait. One answer.

**Step 6H — Branch on goal shape**

Classify Laurent's stated goal:
...
Merge routines + pending + Laurent's goals into one priority-ordered list:
