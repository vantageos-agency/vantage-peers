...
| `block_task` | Mark a task as blocked with optional reason and a `blockedCause` (`peer_task` \| `human` \| `authorisation` \| `other`) naming WHAT is being waited on |
| `fail_task` | Mark a task as **failed** — a terminal state distinct from `done`/`cancelled` — with a mandatory `failureNote`. The only way to record a failure; `update_task` refuses `status="failed"` |
| `add_task_dependency` | Add dependency tasks that must complete first |

</details>

**Blocked-task waiting-on state:** `blockedCause` (optional on `block_task`, defaults server-side to `"other"`) distinguishes a task blocked waiting on a **human answer** from one blocked waiting on an **authorisation** (a merge/publish/approval gate) — the two were previously indistinguishable, collapsed into free text on `blockedOnNobodyReason`. It is a structured discriminator of the cause, not a caller-written state: the waiting-on label a consumer reads is DERIVED from `{status, blockedCause}` via `deriveBlockedWaitingOn` (`convex/tasks.ts`), never written directly. Pre-existing blocked rows are unaffected (read back as `"other"`); no migration ran.

**Task & mission statuses:** `todo` → `in_progress` → `review` → `done` (terminal) or **`failed`** (terminal), plus `blocked` (waiting on a dependency) and `cancelled` (terminal). `failed` is a third terminal, distinct from `done` (succeeded) and `cancelled` (retired before/without attempting the work): a task whose work was genuinely attempted and did not succeed. It is recorded ONLY via `fail_task` (mandatory `failureNote`) — `update_task` refuses `status="failed"` server-side, the same way it refuses `status="blocked"`. The underlying `completionOutcome` (`"succeeded" | "failed"`) is never a caller-supplied arg on `complete_task` or `fail_task`: each tool hardcodes its own value, and `status` is DERIVED from it via `deriveTerminalStatus` (`convex/tasks.ts`) — there is no field for a closer to pick between "finished" and "failed", only a distinct named tool to call. A task already `done`/`cancelled`/`failed` cannot be re-terminated as failed. **A waiter blocked on a task that fails is NOT auto-unblocked**: `fail_task` notifies the waiter's owner that the blocker FAILED (never the success-path "is now done" wording) but leaves the waiter `blocked`, with `blockedOnTaskId` intact — no autonomous pick may treat a failed prerequisite as though it held. Only a blocker reaching `done` sweeps its waiters to `todo`. A task or mission created in error is retired with **`cancelled`** — settable only by its creator, with a mandatory `cancelReason`. A cancelled row is excluded from the `open`/`active` list filters and is never counted as `done`; an already-`done` task (or `complete` mission) cannot be cancelled. Prefer cancelling over deleting — `delete_task` is blocked in production. Pre-existing `done`/`cancelled` rows are left as recorded — no migration reclassifies them, since no signal exists to tell a true failure apart from a real success/cancellation after the fact (see CHANGELOG for the full reasoning).
